...
export function buildRiskQuery(input: RiskQueryInput) {
  ...
  if (input.start_time) {
    query.set("trade_date_start", dayjs(input.start_time).format("YYYYMMDD"));
  }

  if (input.end_time) {
    query.set("trade_date_end", dayjs(input.end_time).format("YYYYMMDD"));
  }

  if (input.account_num) {
    query.set("account_num", input.account_num);
  }
